...
function printTable(title) {
  ...
  w.document.write(`<!DOCTYPE html><html><head><title>${title || 'Print'}</title>
    <style>body{font-family:sans-serif;padding:20px;}table{width:100%;border-collapse:collapse;}th,td{padding:8px 10px;border:1px solid #ddd;font-size:12px;}th{background:#f5f5f5;font-weight:600;}</style>
    </head><body><h2 style="margin-bottom:16px;">${title || ''}</h2>${html}</body></html>`);
  w.document.close();
  w.focus();
