...
async function upload() {
  ...
  console.log(`Found ${rows.length} valid reviewers`);

  // Use upsert to handle potential duplicates on Email
  const { error } = await supabase.from('reviewers').upsert(rows);
  if (error) {
    console.error(`Upload failed:`, error.message);
  } else {
    console.log(`✅ Successfully uploaded ${rows.length} reviewers with 25 columns.`);
  }
}
